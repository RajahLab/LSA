...
Description: This R script contains the code used to analyze data of the Lapses in Sustained Attention (LSA) study for "Post-stimulus Reaction Time at Encoding Predicts Subsequent Source Memory Performance and Modulates Underlying fMRI Activity in Healthy Young Adults" by Vélez Largo, Elshiekh et al. (2023).

Inputs: 

1. "LSA_RT_N38_PrePost.csv": dataset containing trial-by-trial reaction time observations of participants
2. "LSA_IDscores_N38.csv": dataset containing mean accuracy rates, and neuropsychological tests and questionnaires scores of participants


Additional Information:
Run using R version 4.2.3 (2023-03-15) on 2020 MacBook Air (M1 chip).
...
# Merged data set
IDpostpreRT <- mutate(IDpostpreRT, PostRTst = rescale(PostRT),PreRTst = rescale(PreRT), TRIst = rescale(DSSQ_TRI), TUIst = rescale(DSSQ_TUI), CSst = rescale(Scaled_Score_CS),PRst = rescale(WCST_PR_percent), CFQ_Totalst = rescale(CFQ_Total), MASS_LOst = rescale(MASS_LO))


#Scores data set
IDscores <- mutate(IDscores,TRIst = rescale(DSSQ_TRI), TUIst = rescale(DSSQ_TUI), CSst = rescale(Scaled_Score_CS),PRst = rescale(WCST_PR_percent), CFQ_Totalst = rescale(CFQ_Total), MASS_LOst = rescale(MASS_LO))

#Split data sets according to ITI duration 

#ITI duration = 2 secs
IDpostpreRT2 <- mutate(IDpostpreRT2, PostRTst = rescale(PostRT),PreRTst = rescale(PreRT), TRIst = rescale(DSSQ_TRI), TUIst = rescale(DSSQ_TUI), CSst = rescale(Scaled_Score_CS),PRst = rescale(WCST_PR_percent), CFQ_Totalst = rescale(CFQ_Total), MASS_LOst = rescale(MASS_LO))

#ITI duration = 4 secs
IDpostpreRT4 <- mutate(IDpostpreRT4, PostRTst = rescale(PostRT),PreRTst = rescale(PreRT), TRIst = rescale(DSSQ_TRI), TUIst = rescale(DSSQ_TUI), CSst = rescale(Scaled_Score_CS),PRst = rescale(WCST_PR_percent), CFQ_Totalst = rescale(CFQ_Total), MASS_LOst = rescale(MASS_LO))

#ITI duration = 6 secs
IDpostpreRT6 <- mutate(IDpostpreRT6, PostRTst = rescale(PostRT),PreRTst = rescale(PreRT), TRIst = rescale(DSSQ_TRI), TUIst = rescale(DSSQ_TUI), CSst = rescale(Scaled_Score_CS),PRst = rescale(WCST_PR_percent), CFQ_Totalst = rescale(CFQ_Total), MASS_LOst = rescale(MASS_LO))

```



#MAIN PRE/POST-STIMULUS RT MODEL
```{r Fit main model}
...
#8.MASS_LO = Score of Mindful Attention Awareness Scale-Lapses Only  
...
glm_prepost2 <- glmer(Source_Acc_coded ~  PostRTst + PreRTst  + TUIst + TRIst + CSst  + PRst  + CFQ_Totalst  + MASS_LOst  +  PostRTst : TUIst + PostRTst : TRIst + PostRTst : CSst  + PostRTst : PRst  +  PostRTst : CFQ_Totalst  + PostRTst : MASS_LOst + (1 |ID) + (1|Stimuli), data=IDpostpreRT, family='binomial', control = glmerControl(optimizer = "bobyqa"))
...
glmd <- glmer(Source_Acc_coded ~   PreRTst  + TUIst + TRIst + CSst  + PRst  + CFQ_Totalst  + MASS_LOst  + (1 |ID) + (1|Stimuli), data=IDpostpreRT, family='binomial', control = glmerControl(optimizer = "bobyqa"))
...
glme <- glmer(Source_Acc_coded ~  PostRTst + PreRTst  + TRIst + CSst  + PRst  + CFQ_Totalst  + MASS_LOst  +  PostRTst : TRIst + PostRTst : CSst  + PostRTst : PRst  +  PostRTst : CFQ_Totalst  + PostRTst : MASS_LOst + (1 |ID) + (1|Stimuli), data=IDpostpreRT, family='binomial', control = glmerControl(optimizer = "bobyqa"))
...
#MASSLO: PostRT 
drop1(glm_prepost2, "PostRTst:MASS_LOst", test = "Chisq")
...
margins(glm_prepost1)  

margins(glm_prepost2)
...
margins(glm_prepost1.2)  

margins(glm_prepost1.4)
...
glm_postsec1.2 <- glmer(Source_Acc_coded ~  PostRTst + PreRTst  + TUIst + TRIst + CSst  + PRst  + CFQ_Totalst  + MASS_LOst  +  PostRTst : TUIst + PostRTst : TRIst + PostRTst : CSst  + PostRTst : PRst  +  PostRTst : CFQ_Totalst  + PostRTst : MASS_LOst + (1 |ID) + (1|Stimuli), data=IDpostpreRT2, family='binomial', control = glmerControl(optimizer = "bobyqa"))


#ITI = 4 seconds - Similar effects compared to initial secondary model
glm_postsec1.4 <- glmer(Source_Acc_coded ~  PostRTst + PreRTst  + TUIst + TRIst + CSst  + PRst  + CFQ_Totalst  + MASS_LOst  +  PostRTst : TUIst + PostRTst : TRIst + PostRTst : CSst  + PostRTst : PRst  +  PostRTst : CFQ_Totalst  + PostRTst : MASS_LOst + (1 |ID) + (1|Stimuli), data=IDpostpreRT4, family='binomial', control = glmerControl(optimizer = "bobyqa"))
...
glm1.4a <- glmer(Source_Acc_coded ~   PreRTst  + TUIst + TRIst + CSst  + PRst  + CFQ_Totalst  + MASS_LOst  + (1 |ID) + (1|Stimuli), data=IDpostpreRT4, family='binomial', control = glmerControl(optimizer = "bobyqa"))
...
glm1.4b <- glmer(Source_Acc_coded ~  PostRTst + PreRTst  + TRIst + CSst  + PRst  + CFQ_Totalst  + MASS_LOst  +  PostRTst : TRIst + PostRTst : CSst  + PostRTst : PRst  +  PostRTst : CFQ_Totalst  + PostRTst : MASS_LOst + (1 |ID) + (1|Stimuli), data=IDpostpreRT4, family='binomial', control = glmerControl(optimizer = "bobyqa"))
